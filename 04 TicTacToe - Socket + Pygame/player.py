...
def get_user_name():
    ...
    color = (0,0,0)
    color_light = (250,170,170)
    color_dark = (100,100,100)
    width = screen.get_width()
    height = screen.get_height()
    input_rect = pygame.Rect(180, 120, 150, 20)
    user_name = ''
    smallfont = pygame.font.SysFont('black',35)
    base_font = pygame.font.Font(None, 32)
    text = smallfont.render('submit' , True , color)
    running = True
    ...
    while running:
        for ev in pygame.event.get():
            ...
            if ev.type == pygame.MOUSEBUTTONDOWN:
                if 190 <= mouse[0] <= 190+180 and 200 <= mouse[1] <= 260:
                     if user_name != '':
                        running = False
        ...
        titleColor = (0, 0, 0)
        title = smallfont.render("please inter your user name", True, titleColor)
        screen.blit(title, (120, 50))

        mouse = pygame.mouse.get_pos()

        if 190 <= mouse[0] <= 190+180 and 200 <= mouse[1] <= 260:
            pygame.draw.rect(screen,color_light,[190,200,180,60])
        else:
            pygame.draw.rect(screen,color_dark,[190,200,180,60])

        pygame.draw.rect(screen, pygame.Color('#e8e8e8'), input_rect)
        text_surface = base_font.render(user_name, True, (0, 0, 0))
        screen.blit(text_surface, (input_rect.x+5, input_rect.y+14))
        input_rect.w = max(200, text_surface.get_width()+10)
        input_rect.h = 50
        screen.blit(text , (240,218))
